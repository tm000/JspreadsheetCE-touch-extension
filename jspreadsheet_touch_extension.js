var touchExtension = (() => {
	...
	tex.setup = function(spreadsheet, optVal = {}) {
			...
			if (jexcel.current.options.contextMenu) {
				...
				jexcel.timeControl_tex = setTimeout(function() {
					if (jexcel.current && jexcel.current.contextMenu) {
						...
						const el = jexcel.current.content.parentNode.querySelector('.jcontextmenu');
		...
		function rszHandleTouchend(e) {
			if (jexcel.current.resizing) {
				if (jexcel.current.selectedRow) {
					let rowId = jexcel.current.selectedRow;
					let cell = jexcel.current.getCellFromCoords(0, rowId);
					cell.parentNode.classList.remove('resizing');
				} else {
					let columnId = jexcel.current.selectedHeader;
					jexcel.current.headers[columnId].classList.remove('resizing');
					for (var i = 0; i < jexcel.current.records.length; i++) {
						if (jexcel.current.records[i][columnId]) {
							jexcel.current.records[i][columnId].classList.remove('resizing');
						}
					}
				}
		...
		var isTouch = false;
		jexcel.current.content.addEventListener("touchstart", (e) => {
			isTouch = true;
		...
		jexcel.current.content.addEventListener("touchend", (e) => {
			if (jexcel.current && !jexcel.current.edition) {
				// For when mouse events do not occur and only touch events occur
				showSelectionHandle();
			}
		});
		jexcel.current.content.addEventListener("mousedown", (e) => {
			if (!isTouch) {
				hideHandles();
			}
		});
		jexcel.current.content.addEventListener("mouseup", (e) => {
		...
		jexcel.current.contextMenu.contextmenu.close = function() {
			const el = spreadsheet.content.parentNode.querySelector('.jcontextmenu-focus');
			if (el) {
